...
5. [How do I use bootstrap.dat with cttd?](#Importing)
...
**NOTE:** Using bootstrap.dat is entirely optional.  Cttd will download the
...
<a name="Importing" />
### 5. How do I use bootstrap.dat with cttd?

cttd comes with a separate utility named `addblock` which can be used to import
...
1. Stop cttd if it is already running.  This is required since addblock needs to
   access the database used by cttd and it will be locked if cttd is using it.
...
**Windows:**
```bat
C:\> "%PROGRAMFILES%\Cttd Suite\Cttd\addblock" -i C:\Path\To\bootstrap.dat
```
