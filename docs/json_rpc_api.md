...
cttd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between cttd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, cttd intentionally splits the wallet and chain
  services into independent processes.  See the blog post
  [here](https://blog.conformal.com/cttd-not-your-moms-bitcoin-daemon/) for
  further details on why they were separated.  This means that if you are
  talking directly to cttd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [cttwallet](https://github.com/btcsuite/cttwallet).
* cttd is secure by default which means that the RPC connection is TLS-enabled
  by default
* cttd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for cttd RPC and are used by applications
such as [cttwallet](https://github.com/btcsuite/cttwallet) for inter-process
communication with cttd.  The websocket connection endpoint for cttd is
...
during the development of cttwallet.
...
The cttd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
cttd is `wss://your_ip_or_domain:8334/ws`.
...
to a cttd RPC server:

* **rpcuser** is the full-access username configured for the cttd RPC server
* **rpcpass** is the full-access password configured for the cttd RPC server
* **rpclimituser** is the limited username configured for the cttd RPC server
* **rpclimitpass** is the limited password configured for the cttd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the cttd
  server is configured with.  It is automatically generated by cttd and placed
  in the cttd home directory (which is typically `%LOCALAPPDATA%\Cttd` on
  Windows and `~/.cttd` on POSIX-like OSes)

**NOTE:** As mentioned above, cttd is secure by default which means the RPC
...
The cttd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
cttd comes with a separate utility named `btcctl` which can be used to issue
these RPC commands via HTTP POST requests to cttd after configuring it with the
...
|23|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><font color="orange">NOTE: Since cttd does not have the wallet integrated to provide payment addresses, cttd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.</font>|
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">cttd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since cttd does not have the wallet integrated to provide payment addresses, cttd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown cttd.|
|29|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|30|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since cttd does not have a wallet integrated, cttd will only return whether the address is valid or not.|
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a cttd extension**</font>|
...
|Notes|NOTE: Since cttd does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in cttwallet for a version which includes that information.|
...
|Description|Returns information about a transaction given its hash.|
|Notes|<font color="orange">NOTE: Since cttd does not have the wallet integrated to provide payment addresses, cttd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.</font>
...
|Notes|<font color="orange">Since cttd does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since cttd does not have the wallet integrated to provide payment addresses, cttd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">cttd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown cttd.|
|Returns|`"cttd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For cttd this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">Cttd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for cttd.</font>|
...
The following is an overview of the RPC methods which are implemented by cttd, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network cttd is running on.|None|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `CTTD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|string|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR CTTD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get bitcoin network cttd is running on.|
|Returns|numeric|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this cttd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
cttd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll cttd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[cttrpcclient](https://github.com/btcsuite/cttrpcclient) package.
...
[cttrpcclient](https://github.com/btcsuite/cttrpcclient) package to connect with
a cttd instance via Websockets, issues [getblockcount](#getblockcount) to
...
import (
	"github.com/btcsuite/cttrpcclient"
...
func main() {
	...
	// generated by cttd when it starts the RPC server and doesn't already
	// have one.
	cttdHomeDir := cttutil.AppDataDir("cttd", false)
	certs, err := ioutil.ReadFile(filepath.Join(cttdHomeDir, "rpc.cert"))
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[cttrpcclient](https://github.com/btcsuite/cttrpcclient) package to connect with
a cttd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[cttrpcclient](https://github.com/btcsuite/cttrpcclient) package to connect with
a cttd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a cttd instance,
...
// generated by cttd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/cttd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The cttd generated certificate acts as
