...
// newAddressPubKey returns a new cttutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) cttutil.Address {
	addr, err := cttutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new cttutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) cttutil.Address {
	addr, err := cttutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new cttutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) cttutil.Address {
	addr, err := cttutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the cttutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the cttutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := cttutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := cttutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := cttutil.NewAddressPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := cttutil.NewAddressPubKey(decodeHex("03b0bd"+
	...
	p2pkUncompressedMain, err := cttutil.NewAddressPubKey(decodeHex("0411db"+
	...
	tests := []struct {
		...
		{(*cttutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*cttutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*cttutil.AddressPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := cttutil.NewAddressPubKey(decodeHex("02192d7"+
	...
	p2pkUncompressedMain, err := cttutil.NewAddressPubKey(decodeHex("0411d"+
	...
	tests := []struct {
		keys      []*cttutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*cttutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
