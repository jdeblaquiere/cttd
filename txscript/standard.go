...
// specified address.
func PayToAddrScript(addr cttutil.Address) ([]byte, error) {
...
func MultiSigScript(pubkeys []*cttutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []cttutil.Address, int, error) {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := cttutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := cttutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := cttutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]cttutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := cttutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
