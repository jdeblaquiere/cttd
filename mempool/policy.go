...
const (
	...
	DefaultMinRelayTxFee = cttutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee cttutil.Amount) int64 {
	...
	if minFee < 0 || minFee > cttutil.MaxSatoshi {
		minFee = cttutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *cttutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee cttutil.Amount) bool {
...
func checkTransactionStandard(tx *cttutil.Tx, height int32, timeSource blockchain.MedianTimeSource, minRelayTxFee cttutil.Amount) error {
