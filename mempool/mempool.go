...
const (
	...
	MinHighPriority = cttutil.SatoshiPerBitcoin * 144.0 / 250
...
type Config struct {
	...
	FetchUtxoView func(*cttutil.Tx) (*blockchain.UtxoViewpoint, error)
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*cttutil.Tx
	orphansByPrev map[chainhash.Hash]map[chainhash.Hash]*cttutil.Tx
	outpoints     map[wire.OutPoint]*cttutil.Tx
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *cttutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		if _, exists := mp.orphansByPrev[originTxHash]; !exists {
			mp.orphansByPrev[originTxHash] =
				make(map[chainhash.Hash]*cttutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *cttutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *cttutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *cttutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *cttutil.Tx, height int32, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *cttutil.Tx) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *cttutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*cttutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *cttutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	txHash := tx.Hash()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(hash *chainhash.Hash) []*cttutil.Tx {
	var acceptedTxns []*cttutil.Tx
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *cttutil.Tx, allowOrphan, rateLimit bool) ([]*cttutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*cttutil.Tx, len(newTxs)+1)
...
func (mp *TxPool) RawMempoolVerbose() map[string]*btcjson.GetRawMempoolVerboseResult {
	...
	for _, desc := range mp.pool {
		...
		mpd := &btcjson.GetRawMempoolVerboseResult{
			...
			Fee:              cttutil.Amount(desc.Fee).ToBTC(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*cttutil.Tx),
		orphansByPrev: make(map[chainhash.Hash]map[chainhash.Hash]*cttutil.Tx),
		outpoints:     make(map[wire.OutPoint]*cttutil.Tx),
	}
}
