...
const (
	// svcName is the name of cttd service.
	svcName = "cttdsvc"
...
// logServiceStartOfDay logs information about cttd when the main server has
...
// cttdService houses the main service handler which handles all service
// updates and launching cttdMain.
type cttdService struct{}
...
// long-running cttdMain (which is the real meat of cttd), handles service
// change requests, and notifies the service control manager of changes.
func (s *cttdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start cttdMain in a separate goroutine so the service can start
...
// installService attempts to install the cttd service.  Typically this should
...
// removeService attempts to uninstall the cttd service.  Typically this should
...
// startService attempts to start the cttd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &cttdService{})
	if err != nil {
