...
func IsCoinBase(tx *cttutil.Tx) bool {
	return IsCoinBaseTx(tx.MsgTx())
}

// IsFinalizedTransaction determines whether or not a transaction is finalized.
func IsFinalizedTransaction(tx *cttutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CalcBlockSubsidy(height int32, chainParams *chaincfg.Params) int64 {
	...
	if shl == 0 {
		return baseSubsidyCoins * cttutil.SatoshiPerBitcoin
	}

	// Equivalent to: baseSubsidy / 2^(height/subsidyHalvingInterval)
	//return baseSubsidy >> uint(height/chainParams.SubsidyReductionInterval)
    return ((baseSubsidyCoins * shl) / (int64(height) + shl)) * cttutil.SatoshiPerBitcoin
}

// CheckTransactionSanity performs some preliminary checks on a transaction to
// ensure it is sane.  These checks are context free.
func CheckTransactionSanity(tx *cttutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > cttutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *cttutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *cttutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *cttutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
// The flags do not modify the behavior of this function directly, however they
// are needed to pass along to checkBlockHeaderSanity.
func checkBlockSanity(block *cttutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags, hc *ciphrtxt.HeaderCache) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *cttutil.Block, powLimit *big.Int, timeSource MedianTimeSource, hc *ciphrtxt.HeaderCache) error {
...
func ExtractCoinbaseHeight(coinbaseTx *cttutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *cttutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *cttutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *cttutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *cttutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", cttutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > cttutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				cttutil.Amount(originTxSatoshi),
				cttutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > cttutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *cttutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
func (b *BlockChain) CheckConnectBlock(block *cttutil.Block) error {
	b.chainLock.Lock()
