...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockAccepted:
		...
		block, ok := notification.Data.(*cttutil.Block)
		if !ok {
...
// queue.
func (b *blockManager) QueueTx(tx *cttutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *cttutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *cttutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		cttdLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipeDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		cttdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		cttdLog.Infof("Creating block database in memory.")
	...
	cttdLog.Infof("Loading block database from '%s'", dbPath)
	...
	cttdLog.Info("Block database loaded")
	return db, nil
}
