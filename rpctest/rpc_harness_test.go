...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt cttutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(cttutil.Amount(5 * cttutil.SatoshiPerBitcoin))
	...
	txid = genSpend(cttutil.Amount(500 * cttutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(cttutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*cttutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, cttutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := cttutil.Amount(250 * cttutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = cttutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := cttutil.Amount(50 * cttutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := cttutil.Amount(numMatureOutputs * 50 * cttutil.SatoshiPerBitcoin)
