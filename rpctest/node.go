...
import (
	...
	rpc "github.com/jadeblaquiere/cttrpcclient"
	"github.com/jadeblaquiere/cttutil"
)

// nodeConfig contains all the args, and data required to launch a cttd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	a := &nodeConfig{
		listen:    "127.0.0.1:7764",
		rpcListen: "127.0.0.1:7765",
		rpcUser:   "user",
		rpcPass:   "pass",
		extra:     extra,
		prefix:    prefix,
...
// arguments returns an array of arguments that be used to launch the cttd
// process.
...
// command returns the exec.Cmd which will be used to start the cttd process.
...
// to the cttd process that is launched via Start().
...
// as the base for the log and data directories for cttd.
...
// start creates a new cttd process, and writes its pid in a file reserved for
...
// stop interrupts the running cttd process process, and waits until it exits
...
// shutdown terminates the running cttd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := cttutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
