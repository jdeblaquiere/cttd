...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *cttutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *cttutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *cttutil.Tx, isNew bool) {
...
// Notification types
type notificationBlockConnected cttutil.Block
type notificationBlockDisconnected cttutil.Block
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *cttutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *cttutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, cttutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func txHexString(tx *cttutil.Tx) string {
...
func blockDetails(block *cttutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *cttutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *cttutil.Tx, block *cttutil.Block) {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *cttutil.Tx, block *cttutil.Block) {
...
func checkAddressValidity(addrs []string) error {
	for _, addr := range addrs {
		_, err := cttutil.DecodeAddress(addr, activeNetParams.Params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *cttutil.Block) {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *cttutil.Block) error {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	for _, addrStr := range cmd.Addresses {
		addr, err := cttutil.DecodeAddress(addrStr, activeNetParams.Params)
		if err != nil {
			jsonErr := btcjson.RPCError{
				Code: btcjson.ErrRPCInvalidAddressOrKey,
...
fetchRange:
	...
	// Notify websocket client of the finished rescan.  Due to how cttd
